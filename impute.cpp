	...
	while((s=ti_read(t,	iter,	&len))!=0){
		...
		for(uint	m=0;	m<mn;	m++)	if(site[m].chr==chr&&site[m].pos==pos&&site[m].all==all){
			for(uint	i=0;	i<vin;	i++){
				...
				if(st[0]=='0'&&st[2]=='0'){	pa*=conf;	p[0]*=rest;	p[1]*=rest;	}
				else	if((st[0]=='0'&&st[2]=='1')||(st[0]=='1'&&st[2]=='0')){	pa*=rest;	p[0]*=conf;	p[1]*=rest;	}
				else	if(st[0]=='1'&&st[2]=='1'){	pa*=rest;	p[0]*=rest;	p[1]*=conf;	}
...
void	Impute::initialize(void){
	...
	hsum.assign(hn*mn,	0);	pare.assign(in*in,	0);
	...
	for(uint	i=0;	i<in;	i++){
		...
		for(uint	m=0;	m<mn;	m++,	t+=3,	e+=4,	p+=hn){
			...
			if(is_x&&male.find(name[i])!=male.end()&&!is_par[m]){
				t[0]=fmaxf(1-p[0]-p[1],0.0f);	t[1]=0;	t[2]=fmaxf(p[1],0.0f);
				if(t[0]+t[2]){	t[0]/=t[0]+t[2];	t[2]=1-t[0];	}
				else	t[0]=t[2]=0.5;
			}
			else{
...
fast	Impute::solve(uint	I,	uint	&N,	fast	S,	bool	P){
	uint	p[4];
	for(uint	j=0;	j<4;	j++){	
...
void	Impute::estimate(void){
	...
	for(uint	n=0;	n<bn+sn;	n++){
		fast	sum=0,	pen=fminf(2*(n+1.0f)/bn,	1),	iter=0;	pen*=pen;
		for(uint	i=0;	i<in;	i++){
			uint	len=nn*in;
			sum+=solve(i,	len,	pen,	n>=bn);
			iter+=len;
		}
		swap(hnew,	haps);
		if(n>=bn)	for(uint	i=0;	i<in;	i++)	replace(i);
...
int	main(int	ac,	char	**av){
	Impute::bn=56;	Impute::sn=200;	Impute::nn=2;	Impute::density=1.0;	Impute::conf=0.9998;	Impute::is_x=false;	Impute::is_y=false;
	uint	threads=0;
	vector<string>	file;
	...
	while((opt=getopt(ac,	av,	"d:b:l:m:n:t:v:c:x:"))>=0){
		switch(opt) {
		case	'd':	Impute::density=atof(optarg);	break;
		case	'b':	Impute::bn=atoi(optarg);	break;
		case	'm':	Impute::sn=atoi(optarg);	break;
		case	'n':	Impute::nn=atoi(optarg);	break;
		case	't':	threads=atoi(optarg);	break;
		case	'v':	Impute::vcf_file.push_back(optarg);	break;
		case	'c':	Impute::conf=atof(optarg);	break;
		case	'x':	Impute::is_x=true;	Impute::gender(optarg);	break;
		case	'l':{
			char	temp[256];
	...
	for(uint	i=0;	i<fn;	i++){
		...
		Impute	lp;
		if(!lp.load_bin(file[i].c_str())){	cerr<<"fail to load "<<file[i]<<endl;	continue;	}
